...
function ReconcileCard({ item }) {
  ...
  return (
    <div className="rounded-xl bg-white border border-[#1A1A1A]/10 shadow-sm overflow-hidden">
      ...
      {/* the AI match proposal */}
      <div className="px-3 py-2.5" style={{ backgroundColor: 'rgba(91,91,214,0.055)', borderTop: '1px solid rgba(91,91,214,0.16)' }}>
        <div className="flex items-center justify-between gap-2 mb-1.5">
          <span className="flex items-center gap-1 text-[8.5px] font-bold uppercase tracking-wide" style={{ color: C.indigo }}>
            <Sparkles className="w-2.5 h-2.5" /> Likely match
          </span>
          <span className="text-[8.5px] font-bold tabular-nums" style={{ color: C.indigo }}>{item.match.confidence}% confident</span>
        </div>
        <div className="text-[11px] font-bold text-[#1A1A1A] mb-1">PO {item.match.po}</div>
        <p className="text-[9.5px] leading-relaxed text-[#1A1A1A]/60">{item.match.note}</p>
        <div className="flex gap-1.5 mt-2.5">
          <span className="flex-1 flex items-center justify-center gap-1 rounded-md py-1.5 text-[9.5px] font-bold text-white" style={{ backgroundColor: C.green }}>
            <Check className="w-3 h-3" /> Confirm
          </span>
          <span className="flex items-center justify-center gap-1 rounded-md px-2.5 py-1.5 text-[9.5px] font-semibold text-[#1A1A1A]/45 border border-[#1A1A1A]/12">
            <X className="w-3 h-3" /> Not a match
          </span>
        </div>
      </div>
    </div>
